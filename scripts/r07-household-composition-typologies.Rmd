...
2. I then selected a small selection of the 250 plus survey variables to examine the patterns exhibited by these selected typologies, since it would be a thousand pages of material otherwise. 
...
If required I could also do most of these summaries on the 2009 census sample, which would allow for a comparison of our sample with the population as a whole (or perhaps just the relevant provinces). 

Feel free to:

* suggest another way of classifying households
* select topics or questions where you are interested in the differences between different household types. 
...
par(mfrow = c(1,1))
FunNominalBarplot(i = c(17), 
       tab = ds.hohh,
       group.by = "hh.type.fam", 6)

FunNominalBarplot(i = c(18), 
       tab = ds.hohh,
       group.by = "hh.type.fam", 6)

FunNominalBarplot(i = c(19), 
       tab = ds.hohh,
       group.by = "hh.type.fam", 6)
```

...
# # check number of typos in insterviewer names
# table(ds$cnhaptin, ds$nhaptin) # 15
# nrow(ds) - sum(table(ds$cphongvan, ds$phongvan)[,c(2,6, 11,13,16, 19, 23,21, 31, 34)]) #100 

description(ds$cnhaptin) <- "Code of supervisor"
annotation(ds$n1)["comment"] <- "This is a guess at the meaning of the variable as labels were duplicate with cphongvan"

## Annonimyze the data:
FunRemove(c("hhhead", "respondent", "phongvan", "nhaptin"))
...
# version:1.4, 21.3 - a fourth name variable, supervisor, also removed. 